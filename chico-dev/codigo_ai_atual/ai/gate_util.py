...
import rospy
from util import *
from robosub_msgs.msg import control, DetectionArray, Euler
...
class move_within_the_right_angle (SubscribeState):
    def __init__(self, poll_rate = 10):
        SubscribeState.__init__(self, "/pretty/orientation", Euler,
                               self.callback, outcomes=['success', 'wrong_direction'])
        #self.yaw_error_goal = rospy.get_param("ai/turn_to_gate/yaw_error_goal")
        self.yaw_error_goal = 0.5
        self._poll_rate = rospy.Rate(poll_rate)
        self.forward_speed = rospy.get_param("ai/move_to_gate/forward_speed")


    def callback(self, angles, userdata):
        c = control_wrapper()
        c.levelOut()
        yaw = angles.yaw
        c.diveAbsolute(-1.6)
        ...
        #if abs(32 - yaw) < self.yaw_error_goal:            
        #    c.publish()
        if abs(32 - yaw) > self.yaw_error_goal:            
            self._poll_rate.sleep()
            self.exit('wrong_direction')
...
class strafe_for_duration(smach.State):
    ...
    def execute(self, userdata):
        ...
        while not rospy.is_shutdown() and rospy.Time.now() < exit_time:
            c.publish()
            self._poll_rate.sleep()
        #passa o erro target pro controller     
        c.strafeLeftError(0)
        c.publish()
        self._poll_rate.sleep()
        return 'success'
...
# State for turning the robot to the direction of the gate in order to correct the initial angulation
class turn_to_gate(SubscribeState):
    def __init__(self, poll_rate = 10):
        SubscribeState.__init__(self, "/pretty/orientation", Euler,
                               self.callback, outcomes=['success', 'wrong_direction'])
        #self.yaw_error_goal = rospy.get_param("ai/turn_to_gate/yaw_error_goal")
